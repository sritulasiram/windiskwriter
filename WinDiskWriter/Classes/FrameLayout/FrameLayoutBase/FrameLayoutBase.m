...
- (BOOL)containsView: (NSView * _Nonnull)nsView {
    for (FrameLayoutElement *element in self.layoutElementsArray) {
        if (element.nsView == nsView) {
            return YES;
        }
    }
    return NO;
...
- (void)removeView: (NSView * _Nonnull)nsView {
    FrameLayoutElement *targetElement = nil;
    for (FrameLayoutElement *element in self.layoutElementsArray) {
        if (element.nsView == nsView) {
    ...
    if (targetElement != nil) {
        [self.layoutElementsArray removeObject:targetElement];
        [self.sortedElementsArray removeObject:targetElement];
        [targetElement.nsView removeFromSuperview];
        [self applyHugFrames];
        [self setNeedsDisplay:YES];
    }
}
