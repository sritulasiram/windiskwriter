...
+ (BOOL)detachMountedImageAtMountPoint: (NSString *_Nonnull)mountPoint
                                 error: (NSError *_Nullable *_Nullable)error {
    if (mountPoint == NULL || mountPoint.length == 0 || [mountPoint isEqualToString:@"/"]) {
        return YES;
    }
    
    NSException *detachException = NULL;
    CommandLineData *commandLineData = [CommandLine execute: @"/usr/bin/hdiutil"
    ...
    return commandLineData.terminationStatus == EXIT_SUCCESS;
}
