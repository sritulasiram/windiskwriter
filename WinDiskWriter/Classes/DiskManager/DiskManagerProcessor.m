...
- (BOOL)diskUtilEraseVolumeWithFilesystem: (Filesystem)filesystem
    ...
    // Explicitly unmount volume first to avoid Error -69888 (Couldn't unmount disk)
    [CommandLine execute: @"/usr/sbin/diskutil"
               arguments: @[@"unmount", @"force", diskInfo.BSDName]
               exception: NULL];
    ...
    // If it fails with unmount error, retry unmounting forcefully and attempt erase once more
    if (commandLineData.terminationStatus != EXIT_SUCCESS && eraseVolumeException == NULL) {
        NSString *errorPipeOutput = commandLineData.errorData ? [[NSString alloc] initWithData: commandLineData.errorData encoding: NSUTF8StringEncoding] : @"";
        if ([errorPipeOutput containsString: @"-69888"] || [errorPipeOutput.lowercaseString containsString: @"unmount"]) {
            [NSThread sleepForTimeInterval: 1.0];
            [CommandLine execute: @"/usr/sbin/diskutil"
                       arguments: @[@"unmount", @"force", diskInfo.BSDName]
                       exception: NULL];
            
            commandLineData = [CommandLine execute: @"/usr/sbin/diskutil"
                                         arguments: @[@"eraseVolume",
                                                      filesystem,
                                                      newName,
                                                      diskInfo.BSDName
...
- (BOOL)diskUtilEraseDiskWithPartitionScheme: (PartitionScheme _Nonnull)partitionScheme
    ...
    // Explicitly unmount all volumes on the disk first to avoid Error -69888 (Couldn't unmount disk)
    [CommandLine execute: @"/usr/sbin/diskutil"
               arguments: @[@"unmountDisk", @"force", diskInfo.BSDName]
               exception: NULL];
    ...
    // If it fails with an unmount error (-69888), wait momentarily and retry with force unmount
    if (commandLineData.terminationStatus != EXIT_SUCCESS && eraseVolumeException == NULL) {
        NSString *errorPipeOutput = commandLineData.errorData ? [[NSString alloc] initWithData: commandLineData.errorData encoding: NSUTF8StringEncoding] : @"";
        if ([errorPipeOutput containsString: @"-69888"] || [errorPipeOutput.lowercaseString containsString: @"unmount"]) {
            [NSThread sleepForTimeInterval: 1.0];
